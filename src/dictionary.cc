...
// 根据字符串进行hash, hash后若是冲突则线性探索, 找到其对应的hash位置
...
void Dictionary::computeNgrams(const std::string& word,
  ...
  // 去掉其中的一些词进行计算, 生成新想n-gram
  for (size_t i = 0; i < word.size(); i++) {
    std::string ngram;
    // (word[i] & 0xC0) != 0x80) : word[i]是一个单字符或者多字符
    if ((word[i] & 0xC0) == 0x80) continue;
    // word有可能是多个字的词
    for (size_t j = i, n = 1; j < word.size() && n <= args_->maxn; n++) {
      ngram.push_back(word[j++]);
...
void Dictionary::threshold(int64_t t, int64_t tl) {
      sort(words_.begin(), words_.end(), [](const entry& e1, const entry& e2) {
          if (e1.type != e2.type) return e1.type < e2.type;  // 不同类型的词, label词排在后面
          return e1.count > e2.count;  // 同类型的词, 频率大的排在前面
        });
      // 删除阈值以下的词
